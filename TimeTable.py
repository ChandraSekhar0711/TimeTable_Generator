...
emp=""
p=[[emp for i in range(n+5)]for j in range(m+5)] #sec1
q=[[emp for i in range(n+5)]for j in range(m+5)] #sec2
freq=[]
freq1=[]
...
#Section1 Allocation starts from here.....
def sec1():
    def default_periods():
        def exams():
                i=2
                for j in range(0,i):
                        p[i][j]=default[0] #allocating exams for Section1
                        q[i][j]=default[0] #allocating exams for Section2
        exams()

        def rev():
                i=1
                for j in range(4,n):
                        p[i][j]=default[1]  #allocating revision for Section1
                        q[i][j]=default[1]  #allocating revision for Section2
        rev()

        def elec(): #Elective is also a subject
            i=2
            count=0
            while count<5:
                ...
                if p[k][s]!=emp or q[k][s]!=emp :
                  ...
                  p[k][s]=default[i]  #allocating elective for Section1
                  q[k][s]=default[i]  #allocating elective for Section2
                  count=count+1
        elec()
    ...
    def lab_periods(): #Allocating Labs for Section1
        def lab():
                i=0
                count =0
                while count<9:
    ...
    def subj():
        def subjects(): #Allocating main_subjects for Section1
          i=0
          count=0
          while count<20:
...
def sec2():
    ...
    def labs1(): #Allocating labs for Section2
            i=0
            count=0
            while count<9:
                    k=random.randint(0,5)
    ...
    def subjects1(): #Allocating main_subjects for Section2
      i=0
      count=0
      while count<20:
